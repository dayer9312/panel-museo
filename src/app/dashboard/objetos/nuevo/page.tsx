...
export default function NuevoObjetoPage() {
  ...
  useEffect(() => {
    const fetchSalas = async () => {
      try {
        const res = await fetch("http://localhost:3001/sala");
  ...
  const guardarObjeto = async (e: React.FormEvent) => {
    ...
    try {
      const respuesta = await fetch("http://localhost:3001/objeto", {
        method: "POST",
        ...
        <form onSubmit={guardarObjeto} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="col-span-2">
              ...
              <input type="text" required className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-600 outline-none text-gray-900" placeholder="Ej: Espada de Simón Bolívar" value={titulo} onChange={(e) => setTitulo(e.target.value)} />
            </div>

            {/* Autor */}
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-1">Autor / Creador</label>
              <input type="text" className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-600 outline-none text-gray-900" placeholder="Ej: Desconocido" value={autor} onChange={(e) => setAutor(e.target.value)} />
            </div>

            {/* Año */}
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-1">Año o Época</label>
              <input type="text" className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-600 outline-none text-gray-900" placeholder="Ej: 1825 o Siglo XIX" value={anio} onChange={(e) => setAnio(e.target.value)} />
            </div>
          </div>
          ...
          <div>
            ...
            <textarea required rows={4} className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-600 outline-none resize-none text-gray-900" placeholder="Detalla la historia y relevancia de este objeto..." value={descripcion} onChange={(e) => setDescripcion(e.target.value)} />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-4 bg-slate-50 rounded-lg border border-slate-100">
            ...
            <div>
              ...
              {cargandoSalas ? (
                ...
                <select 
                  required
                  className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-600 outline-none bg-white text-gray-900"
                  value={idSala}
